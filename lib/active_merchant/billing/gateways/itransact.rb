    ...
    class ItransactGateway < Gateway
      ...
      def add_invoice(xml, money, options)
        ...
        if options[:order_items].blank?
          xml.Total to_dollars(money).to_s unless money.nil? || money < 0.01
          ...
          xml.OrderItems {
            options[:order_items].each do |item|
              xml.Item {
                ...
                xml.Cost to_dollars(item[:cost]).to_s
      ...
      def determine_funding_source(payment_source)
        if(payment_source.is_a?(ActiveMerchant::Billing::CreditCard))
          :credit_card
        elsif(payment_source.is_a?(ActiveMerchant::Billing::Check))
          :check
        end
      end
